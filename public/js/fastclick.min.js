...
function clicked(selection, redir) 
{
var selected = document.querySelectorAll('.fastclicked')[0];
	if (selection == selected)
		return;
	if (redir) {
		document.location="battle:selection:" + selection.id;
	}
	selection.className = 'fastclicked';
	selected.className = 'fastclick';
}
window.addEventListener('load', function() { 
	var a = document.querySelectorAll(".fastclick"); 
	for (var i=0;i<a.length;i++) { 
		new FastClick(a[i]); 
		a[i].onclick = function() {clicked(this, true)};
	}
}, false);
